...
#define BUTTON_STEP_DELAY   5
#define BUTTON_STEP_SIZE    1
...
const String programName = "myFP2.DRV8825";
...
void processCommand()
{
  ...
  switch ( cmdval )
  {
    ...
    case 15:  // :15XX#   None    Set MotorSpeed, 00 = Slow, 01 = Med, 02 = Fast
      ...
      flageepromupdate();
      break;
    case 16:  // :16#   None      Set Display in Celsius (LCD or TFT)
...
void loop()
{
  ...
  if ( stepperpower == 1)
  {
    if ( myfocuser.tempcompenabled == 0 )
    {
      ...
      if ( PBVal != 0)
      {
        delay( BUTTON_STEP_DELAY );                             // wait small delay in case user is holding pb down
        ...
        if ( PBVal != 0 )
        {
          switch ( PBVal )                      // now check the pbval using a switch for 1 2 and 3
          {
            case 1:                             // toggle sw1 is ON and 2 is off
              ...
              movestarted = 1;
              isMoving = 1;
              targetPosition = targetPosition - BUTTON_STEP_SIZE;
              if ( targetPosition < 0 )
                targetPosition = 0;
#ifdef LCDDISPLAY
              updatepositionlcd();
