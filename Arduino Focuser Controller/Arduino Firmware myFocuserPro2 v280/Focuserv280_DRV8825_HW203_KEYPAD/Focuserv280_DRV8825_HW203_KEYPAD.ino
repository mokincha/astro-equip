// myfocuser DRV8825 HW203 FULL WITH 4x4 MEMBRANE KEYPAD PCF8274 I2C BOARD AND LCD DISPLAY
// ONLY FOR USE DRV8825 AND EASYDRIVER Driver Board - supports 1.5A at 12VDC and microstepping
// BIPOLAR STEPPER MOTORS ONLY
// SUPPORTS TEMPERATURE PROBE DS18B20, LCD1602/I2C, KEYPAD 4x4, HOME POSITION SWITCH

// JOGGING AND PUSH BUTTONS AND BLUETOOTH NOT SUPPORTED
// HW203 OF DRV8825 BOARD LAYOUT (M0/M2 swapped)
//
...
// Mapping for LCD DISPLAY - MANDATORY
// VCC    5V
// GND    GND
// SDA    A4
// SCL    A5
...
// By default, this is a FULL Controller firmware

// You MUST HAVE AN LCD DISPLAY
...
// To enable support when using a Micro instead of a Nano, uncomment the next line
//#define CONTROLLERISAMICRO 1           // provided by IL, enables the serialEventRun function for a Micro

// do not change, a display is mandatory
#define LCDDISPLAY 1
...
// 2.71
// No changes as this firmware does not use push buttons
...
String strhash = "Press # to exit";
char keypadstr[lenkeypadstr];
const char NO_KEY = '\0';
char keypadvalue;
int keypadmenuline;                   // line number of keypad menu to display
String keypadmenuitems[] = {
  "0=Goto Pos",
  "1=Set Pos",
  "2=Set maxStep",
  "3=Set Stepmode",
...
String programName = "myFP2.DRV8825HW203.KP";
...
void  displaylcd2004pg2()
{
  ...
  if ( myfocuser.stepmode == STEP1 )
    ...
    lcd.print("2");
  else if ( myfocuser.stepmode == STEP4 )
    lcd.print("4");
  else if ( myfocuser.stepmode == STEP8 )
    lcd.print("8");
  else if ( myfocuser.stepmode == STEP16 )
    lcd.print("16");
  else if ( myfocuser.stepmode == STEP32 )
    lcd.print("32");
...
} // end of findds18b20address()
#endif

// keypad stuff START =====================================================
void mylcdheader(String strmsg, bool nl)
{
  lcd.clear();
  if ( nl )
  {
...
// display keypad menu choices
void keypadmenuchoice( int keypadmenuline )
{
  mylcdheader(keypadmenuitems[keypadmenuline], 0);
}
...
void loop()
{
  ...
  char cpkey = NO_KEY;
  // read keypad
  keypadvalue = kpd.get_key();
  // if no key then keypadvalue = '\0';

  if (keypadvalue == '*' )
  {
    // first halt the focuser
    targetPosition = currentPosition;
    isMoving = 0;
    ...
    while ( keypadvalue != '#' )
    {
      ...
      keypadmenuchoice(keypadmenuline);

      // switch depending upon value
      switch ( keypadvalue )
      {
        case 'A' : keypadmenuline--; if ( keypadmenuline < 0 ) keypadmenuline = 9; break; // scroll menu up and wrap around
        case 'B' : keypadmenuline++; if ( keypadmenuline > 9 ) keypadmenuline = 0; break; // scroll menu down and wrap around
        case '0' : // goto position
          {
            long number;
            cpkey = NO_KEY;
            while (cpkey != '#' )
            {
              // a generic get number from keypad is needed as its common for cases 1-3
              mylcdheader("CPos=", 0);
              lcd.print(currentPosition);
              getkeypadstr();
              cpkey = '#';
            ...
            if ( String(keypadstr) != "")
            {
              ...
              if ( number <= 0 )
                number = 0;
              if ( number >= myfocuser.maxstep )
                number = myfocuser.maxstep;
              // update position to move to
              if ( stepperpower == 1)
              {
          ...
          cpkey = '#';
          lcd.setCursor(0, 1);
          lcd.print("Done. Pls wait.");
          olddisplaytimestampNotMoving = millis();      // use a delay by resetting the LCD display timer
          break;

        case '1' : // set position to new value
          {
            long number;
            cpkey = NO_KEY;
            while (cpkey != '#' )
        ...
        case '2' : // set maxstep to new value
          {
            long number;
            cpkey = NO_KEY;
            while (cpkey != '#' )
            ...
            if ( number <= 0 )
              number = 0;
            if ( number >= FOCUSERUPPERLIMIT )
              number = FOCUSERUPPERLIMIT;
            if ( number <= currentPosition )
              number = currentPosition;
            if ( number < FOCUSERLOWERLIMIT )
              number = FOCUSERLOWERLIMIT;
            // update maxstep
            myfocuser.maxstep = number;
            flageepromupdate();
          }
          cpkey = '#';
          lcd.setCursor(0, 1);
        ...
        case '3' : // set stepmode
          ...
          // accept 1=full, 2=half, 4=1/4, 8=1/8, 6=1/16, 3=1/32
          lcd.setCursor( 0, 1 );
          lcd.print("1 2 4 8 6 3");
          cpkey = NO_KEY;
          while (cpkey != '#' )
          {
            cpkey = kpd.get_key();
            switch ( cpkey )
            {
              ...
              case '4': // 1/4
                ...
                mylcdheader("SM =1/4", 1);
                lcd.print(strhash);
                break;
              case '8': // 1/8
                myfocuser.stepmode = 8;
                mylcdheader("SM=1/8", 1);
                lcd.print(strhash);
                break;
              case '6': // 1/16
                myfocuser.stepmode = 16;
                mylcdheader("SM=1/16", 1);
                lcd.print(strhash);
                break;
              case '3': // 1/32
                myfocuser.stepmode = 32;
                mylcdheader("SM=1/32", 1);
                lcd.print(strhash);
                break;
            } // end of switch set temperature resolution
          } // end of wait for # key
          setstepmode(myfocuser.stepmode);
          olddisplaytimestampNotMoving = millis();      // use a delay by resetting the LCD display timer
          flageepromupdate();
        ...
        case '4' : // set Temp Resolution
          {
            // Line 0 displays current temperature resolution setting
            // Line 1 displays available choice – scroll to select desired resolution using A or B then press #
            mylcdheader("TR=", 0);
            switch ( myfocuser.ds18b20resolution )
            {
              case 9:   // 0.5 degree
                lcd.print("0.5d");
                break;
            ...
            while (cpkey != '#' )
            {
              ...
              switch ( cpkey )
              {
                case '0': // 0.0625 degrees
                  myfocuser.ds18b20resolution = 12;
                  mylcdheader("TR=0.0625", 1);
                  lcd.print(strhash);
                  break;
                case '1': // 0.125
                  myfocuser.ds18b20resolution = 11;
                  mylcdheader("TR=0.125", 1);
                  lcd.print(strhash);
                  break;
                case '2': // 0.25
                  myfocuser.ds18b20resolution = 10;
                  mylcdheader("TR=0.25", 1);
                  lcd.print(strhash);
                  break;
                case '5': // 0.5
                  myfocuser.ds18b20resolution = 9;
                  mylcdheader("TR=0.5", 1);
                  lcd.print(strhash);
                  break;
              } // end of switch set temperature resolution
            } // end of wait for # key
        ...
        case '6' : // set Reverse Direction
          {
            mylcdheader("RD=", 0);
            if ( myfocuser.ReverseDirection == 0 )
              lcd.print("OFF");
            else
              lcd.print("ON");
            cpkey = NO_KEY;
        ...
        case '7' : // set tempmode c/f 1=c, 0-f
          {
            mylcdheader("TM=" , 0);
            if ( myfocuser.tempmode == 1 )
              lcd.print("C");
        ...
        case '8' : // Display Firmware
          {
            // print the version then exit immediately
            mylcdheader(programName, 0);
            lcd.setCursor(0, 1);
            lcd.print(programVersion);
            cpkey = '#';
          } // end of case 8
        ...
        case '9' : // set MotorSpeed
          {
            mylcdheader("MS=", 0);
            switch ( myfocuser.motorSpeed )
            {
              case 0: lcd.print("S"); break;
              case 1: lcd.print("M"); break;
              case 2: lcd.print("F"); break;
            }
            lcd.setCursor( 0, 1 );
            lcd.print("0=S 1=M 2=F");
            cpkey = NO_KEY;
            while (cpkey != '#' )
            {
              cpkey = kpd.get_key();
              ...
              {
                case '0': // Slow
                  myfocuser.motorSpeed = 0;
                  mylcdheader("MS=S", 1);
                  lcd.print(strhash);
                  break;
                case '1': // Medium
                  myfocuser.motorSpeed = 1;
                  mylcdheader("MS=M", 1);
                  lcd.print(strhash);
                  break;
                case '2': // Fast
                  myfocuser.motorSpeed = 2;
                  mylcdheader("MS=F", 1);
                  lcd.print(strhash);
                  break;
              } // end of switch for motorspeed
            } // end of wait for # key
          } // end of case 9
          lcd.setCursor(0, 1);
          lcd.print("Done. Pls wait.");
          // necessary because motorSpeed is restored in loop()
          olddisplaytimestampNotMoving = millis();      // use a delay by resetting the LCD display timer
          savedmotorSpeed = myfocuser.motorSpeed;
          cpkey = '#';
          break;
      ...
      delay(40);           // prevents lcd flashing
      // read keypad
      if ( cpkey != '#' )
        keypadvalue = kpd.get_key();
      else
        keypadvalue = '#';
    } // end of while keypadchar != #
    // it is a #
  }
  // keypad stuff END =====================================================
