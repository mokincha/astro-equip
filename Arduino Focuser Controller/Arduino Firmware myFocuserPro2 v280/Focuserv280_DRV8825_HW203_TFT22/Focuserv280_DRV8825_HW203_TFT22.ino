// myfocuser DRV8825 TFT 2.2" FULL (but no push buttons)
// ONLY FOR USE DRV8825 TFT ONLY - supports 1.5A at 12VDC and microstepping
// BIPOLAR STEPPER MOTORS ONLY
// PUSH BUTTONS, BUZZER, LEDS, TEMPERATURE PROBE, TFT 2.2" DISPLAY
// USES HW203 Layout with Nano+DRV8825 but Enable Pin is on A4

// HOMEPOSITION SWITCH NOT SUPPORTED
// ONLY TO BE USED WITH TFT 2.2" DISPLAY - ILI9341, ILI9341 320x240 (40x30)

// THE TFT DESIGN IS NO LONGER SUPPORTED BUT THIS FIRMWARE SUPPORTS UNITS ALREADY BUILT FOR BACKWARD SUPPORT
// RECOMMEND THAT NEW FOCUSERS BE BUILT WITH THE OLED OPTION INSTEAD
...
// ----------------------------------------------------------------------------------------------------------
// PCB BOARDS
// Stripboard Layout Only
// myFocuser_DRV8825_HW203_F_TFT_2_2_schem
// myFocuser_DRV8825_HW203_F_TFT_2_2_Layout
...
// 2.80
// Fixes
// Major fix for focuser not moving correctly
...
#define sclk                10            // these are on the header of the tft as per above
#define din                 11
#define cs                  9
#define dc                  8
#define rst                 12
...
#ifdef TFTDISPLAY
UTFT mytft(QD_TM220, din, sclk, cs, rst, dc); // QD_TM220 is for ILI9341
// insufficient memory to use more than one font, must be uppercase only
extern uint8_t SmallFont[];
#endif

// MUST BE UPPERCASE ONLY
char tempString[12];
char programName[] = "MYFP2.DRV8825HW203.TFT22";
...
void processCommand()
{
  ...
  switch ( cmdval )
  {
    ...
    case 4:     // :04#     FString#  get firmware version string (Fprogram name, version, #)
      replystr = "F" + String(programName);
      sendresponsestr(replystr);
      sendresponsenl();
      replystr = String(programVersion) + "#";
      sendresponsestr(replystr);
      break;
...
#ifdef TFTDISPLAY
// update position on lcd
void updatepositionlcd()
{
  String cp = String(currentPosition);
  String tp = String(targetPosition);
...
void displaylcd()
{
  ...
  if ( myfocuser.stepmode == 1 )
    mytft.print(step1, 160, 80);
  else if ( myfocuser.stepmode == 2 )
    mytft.print(step2, 160, 80);
  else if ( myfocuser.stepmode == 4 )
    mytft.print(step4, 160, 80);
  else if ( myfocuser.stepmode == 8 )
    mytft.print(step8, 160, 80);
  else if ( myfocuser.stepmode == 16 )
    mytft.print(step16, 160, 80);
  else if ( myfocuser.stepmode == 32 )
    mytft.print(step32, 160, 80);
...
#ifdef TFTDISPLAY
  mytft.clrScr();               // clear screen display
#endif
}
