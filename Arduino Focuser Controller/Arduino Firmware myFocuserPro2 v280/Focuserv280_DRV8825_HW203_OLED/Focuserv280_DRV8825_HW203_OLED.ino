...
// USES HW203 Layout (M0/M2 swapped) with Nano+DRV8825+OLED
//
// ONLY TO BE USED WITH OLED 0.96" 128X64 DISPLAY - SSD1306 (see below for connections)
...
// Stripboard/Layout
// myFocuser_DRV8825_HW203_F_OLED_Layout
...
// SDA    A4
// SCL    A5
// NOTE: YOU CANNOT USE A STRAIGHT 4P-4P HEADER CABLE
...
// Infrared Control (uses Timer2) - do not use with Rotary Encoder, requires DRV8825HW203_FIRBT PCB
// Pin 9
...
struct config_t {
  ...
  long pagedisplaytime;           // refresh rate of display - time each page is displayed for
...
char programName[]    = "myFP2.DRV8825HW203.OLED";
...
byte RESWVal;                             // state of Rotary Encoder Switch
...
void processCommand()
{
  ...
  switch ( cmdval )
  {
    ...
    case 45:  // :45#   Gxxx#     Get tswthreshold - value for which stepper slows down at end of its move
      ...
      sendresponsestr(replystr);
      break;
    case 47:  // 47#    Gxxx#     Get if motorspeedchange enabled/disabled
...
#ifdef OLEDDISPLAY
      displayenabled == 0 ? myoled.Display_Off() : myoled.Display_On();
#endif
      break;
...
void setup()
{
  ...
  nlocations = EEPROMSIZE / datasize;  for (int lp1 = 0; lp1 < nlocations; lp1++ )
  {
