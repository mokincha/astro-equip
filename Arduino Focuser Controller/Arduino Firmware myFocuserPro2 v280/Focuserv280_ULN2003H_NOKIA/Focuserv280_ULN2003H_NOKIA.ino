...
// FULL VERSION
// SUPPORTS PUSH BUTTONS, NOKIA, TEMPERATURE PROBE
//
...
// ----------------------------------------------------------------------------------------------------------
// PCB BOARDS
// https://aisler.net/brown_rb/my-project-repository/myfocuserpro2-uln2003-nokia
...
#ifdef L293D
#define SLOWDELAY1          40            // L293D
#define SLOWDELAY2          80            // L293D
#define MEDDELAY1           20            // L293D
#define MEDDELAY2           60            // L293D
#define FASTDELAY1          1             // L293D
#define FASTDELAY2          10            // L293D
// motor speeds in RPM - you need to adjust these depending on the stepper motor you select and the step mode
// Only valid for a single step mode
#define motorSpeedSlowRPM   10            // L293D
#define motorSpeedMedRPM    20            // L293D
#define motorSpeedFastRPM   40            // L293D
#endif

#ifdef ULN2003
#define STEPSPERREVOLUTION  2048          // there are 2048 steps for one rotation of the 28BYJ-48 shaft
#define IN1                 4
#define IN2                 5
#define IN3                 6
#define IN4                 7
#define SLOWDELAY1          20            // ULN2003
#define SLOWDELAY2          12            // ULN2003
#define MEDDELAY1           10            // ULN2003
#define MEDDELAY2           5             // ULN2003
#define FASTDELAY1          4             // ULN2003
#define FASTDELAY2          2            // ULN2003
// motor speeds in RPM - you need to adjust these depending on the stepper motor you select and the step mode
// Only valid for a single step mode
#define motorSpeedSlowRPM   2             // ULN2003
#define motorSpeedMedRPM    5             // ULN2003
#define motorSpeedFastRPM   40            // ULN2003
HalfStepper mystepper(STEPSPERREVOLUTION, 4, 6, 7, 5);
...
const String programName = "myFP2.ULN2003.NOKIA";
const String programVersion = "280";
...
void  displaylcdpg1()
{
  ...
  str = "Posn= " + String(currentPosition);
  lcd.print(str, LEFT, 0);
  str = "Targ= " + String(targetPosition);
  lcd.print(str, LEFT, 8);
  str = "CPwr= ";
  if ( myfocuser.coilPwr )
    str = str + "Y";
  else
    str = str + "N";
  lcd.print(str, LEFT, 16);
  str = "RDir= ";
  if ( myfocuser.ReverseDirection )
    str = str + "Y";
  else
    str = str + "N";
  lcd.print(str, LEFT, 24);
...
void updatepositionlcd()
{
  ...
  LCDScreen = 1;
  lcd.print(tmpStr, LEFT, 0);
  str = "POS=" + String(currentPosition);
  lcd.print(str, LEFT, 0);

  lcd.print(tmpStr, LEFT, 0);
  str = "TAR=" + String(targetPosition);
  lcd.print(str, LEFT, 8);
}
#endif
...
void setup()
{
  ...
  pinMode(IN1, OUTPUT);                   // setup stepper
  pinMode(IN2, OUTPUT);
  pinMode(IN3, OUTPUT);
  pinMode(IN4, OUTPUT);
  ...
  (stepperpowerread > 800) ? stepperpower = 1 : stepperpower = 0;
#endif
  ...
  lcd.clrScr();                        // print the title of program and copyright message to lcd
  lcd.print(programName, LEFT, 0);
  lcd.print(programVersion, LEFT, 8);
#endif
...
#ifdef DEBUG
  Serial.print("- myfocuser.fposition="); Serial.print(myfocuser.fposition); Serial.print("#");
#endif
...
void loop()
{
  ...
  else
  {
    ...
    if ( ((currentMillis - olddisplaytimestampNotMoving) > (myfocuser.pagedisplaytime * 100)) || (currentMillis < olddisplaytimestampNotMoving) )
    {
      ...
      switch ( LCDScreen)
      {
        case 1: displaylcdpg1(); break;
        case 2: displaylcdpg2(); break;
      }
    }
#endif
    ...
    if ( writenow == 1 )
    {
      ...
      long currentMillis = millis();
      if ( (currentMillis - previousMillis) > EEPROMWRITEINTERVAL)
      {
        myfocuser.validdata = VALIDDATAFLAG;
        myfocuser.fposition = currentPosition;
        writeEEPROMNow();    // update values in EEPROM
