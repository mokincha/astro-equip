...
// SUPPORTS TEMPERATURE PROBE, LCD1602/I2C, PUSH BUTTONS, BLUETOOTH, INFRA RED CONTROLLER, ROTARY ENCODER, HOME POSITION SWITCH
...
// BLUETOOTH HC-05 Module wired as follows (not compatible with ESP8266 Wifi)
...
// ROTARY ENCODER (uses Timer2) - do not use with IR, pushbuttons or Bluetooth, requires DRV8825HW203_FRE PCB
// Pins 9 and 10 - rotary encoder
// Pin 11 - switch

// INFRARED CONTROLLER (uses Timer2) - do not use with Rotary Encoder, requires DRV8825HW203_FIRBT PCB
// Pin 9
...
// To enable bluetooth, uncomment the next line -  - Do not use with Rotary Encoder, Infrared or Wifi
#define BLUETOOTH 1
...
// To enable the Rotary Encoder for manual focusing, uncomment the next line - do not use with IR, Bluetooth or Pushbuttons
//#define ROTARYENCODER 1

// To enable the Infrared control for manual focusing, uncomment the next line - do not use with Rotary Encoder or Pushbuttons
//#define INFRARED 1
...
#define WIFIPORTSPEED       9600          // define wifi speed, DO NOT CHANGE
...
char programName[] = "myFP2.DRV8825HW203";
...
#ifdef ROTARYENCODER
#ifdef BLUETOOTH
#halt // Error - You cannot have ROTARY ENCODER and BLUETOOTH enabled at the same time
#endif
#endif

#ifdef ROTARYENCODER
#ifdef INFRARED
#halt // Error - You cannot have ROTARY ENCODER and INFRARED enabled at the same time
#endif
#endif

#ifdef INFRARED
#ifdef BLUETOOTH
#halt // Error - You cannot have INFRARED CONTROLLER and BLUETOOTH enabled at the same time
#endif
#endif

#ifdef ROTARYENCODER
#ifdef PUSHBUTTONS
#halt // Error - You cannot have ROTARY ENCODER and PUSHBUTTONS enabled at the same time
#endif
#endif

#ifdef INFRARED
#ifdef PUSHBUTTONS
#halt // Error - You cannot have INFRARED CONTROLLER and PUSHBUTTONS enabled at the same time
#endif
#endif
...
int readpbswitches(int pinNum)
{
  ...
  digitalWrite(pinNum, HIGH);               // enable 20k internal pullup, 14=A0
...
void processCommand()
{
  ...
  switch ( cmdval )
  {
    ...
    case 66:  // :66#   Kxx#      Get jogging state enabled/disabled
      ...
      sendresponsestr(replystr);
      break;
    case 68:  // :68#   Vxx#      Get jogging direction 0=IN, 1=OUT
      replystr = "V" + String(joggingDirection) + "#";
      sendresponsestr(replystr);
      break;
    case 72: // :72# 3xxx#        Get DelayAfterMove
      replystr = "3" + String(myfocuser.DelayAfterMove) + "#";
      sendresponsestr(replystr);
      break;
    case 74:  // :74#             Get backlash in enabled status
      replystr = "4" + String(myfocuser.backlash_in_enabled) + "#";
      sendresponsestr(replystr);
      break;
    case 76:  // :76#             Get backlash OUT enabled status
      replystr = "5" + String(myfocuser.backlash_out_enabled) + "#";
      sendresponsestr(replystr);
      break;
    case 78: // :78#              Get number of backlash steps IN
      replystr = "6" + String(myfocuser.backlashsteps_in) + "#";
      sendresponsestr(replystr);
      break;
    case 80: // :80#              Get number of backlash steps OUT
      replystr = "7" + String(myfocuser.backlashsteps_out) + "#";
      sendresponsestr(replystr);
      break;
    case 83: // :83#              Get if there is a temperature probe
    ...
    case 27:  // :27#   None      stop a move - like a Halt
      ...
      flageepromupdate();
      break;
    case 30:  // :30xx#   None    set stepmode (1=Full, 2=Half, 4=1/4, 8=1/8, 16=1/16, 32=1/32)
      paramval = decstr2int(param);
...
void  displaylcd2004pg1()
{
  ...
  lcd.setCursor( 0, 2 );
  lcd.print("CPwr=");
  if ( myfocuser.coilPwr == 1) lcd.print("Y"); else lcd.print("N");
  lcd.setCursor( 10, 2 );
  lcd.print("RDir=");
  if ( myfocuser.ReverseDirection == 1 ) lcd.print("Y"); else lcd.print("N");
  lcd.setCursor(0, 3);
  lcd.print("Temp=");
  if ( tprobe1 == 0 )
  {
...
void loop()
{
    ...
    if ( myfocuser.tempcompenabled == 0 )
    {
      if ( isMoving == 0 )                  // do not update if focuser is already moving - just wait
      {
        if (irrecv.decode(&results))
        {
          ...
          newPos = currentPosition + adjpos;  // adjust the target position
          movestarted = 1;
          if ( newPos < 0 )
            newPos = 0;
          if ( newPos > myfocuser.maxstep )
    ...
    if ( myfocuser.tempcompenabled == 0 )
    {
      ...
      if ( isMoving == 0 )
      {
        ...
        if ( lp != 0 )
        {
          ...
          newPos = currentPosition + (myrotaryencoder.getposition() * encswval);
          movestarted = 1;
          if ( newPos < 0 )
            newPos = 0;
          if ( newPos > myfocuser.maxstep )
  ...
  if (targetPosition != currentPosition)
  {
    ...
    {
      isMoving = 1;
      settohome();                          // need to back OUT a little till switch opens and then set position to 0
      isMoving = 0;
    }
    ...
    {
      isMoving = 1;
      movetohome();                         // need to move IN a little till switch CLOSES then
      settohome();                          // need to back OUT a little till switch opens and then set position to 0
      isMoving = 0;
    }
#endif
  ...
  else
  {
    ...
    if ( stepperpower == 1)
    {
      if ( myfocuser.tempcompenabled == 1 )
      {
        ...
        double tempchange = starttempval - ch1tempval;              // if temperature has changed by 1 degree
